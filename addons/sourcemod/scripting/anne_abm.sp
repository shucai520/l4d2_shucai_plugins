...
public void OnPluginStart() {
	...
	RegConsoleCmd("sm_jg", SwitchTeamCmdSc, "Menu/Cmd: <TEAM> | <ID> <TEAM>");
	RegConsoleCmd("sm_c", AssignModelCmdSc, "选择人物");
	RegAdminCmd("sm_mk", MkBotsCmd, ADMFLAG_GENERIC, "Cmd: <N|-N> <TEAM>");
	RegAdminCmd("sm_kb", RmBotsCmd, ADMFLAG_GENERIC, "Cmd: <TEAM> | <N|-N> <TEAM>");
	//RegConsoleCmd("sm_c", ChangeModelCmd, "选择人物"); 
	RegAdminCmd("sm_abms", MainMenuCmd, ADMFLAG_ROOT);

	RegAdminCmd("sm_rs", Command_renshu, ADMFLAG_GENERIC, "最大人类数量.");
	RegConsoleCmd("sm_away", Command_xianzhi, "人类闲置.");
	...
	SetupCvar(g_cvJoinMenu, "abm_joinmenu", "2", "0: Off 1: Admins only 2: Everyone");
...
public Action Command_xianzhi(int client, int args)
{
	GoIdle(client, 1);
	return Plugin_Handled;
}

public Action Command_renshu(int client, int args)
{
	if(GetCmdArgs() < 1)
	{
		return Plugin_Handled;
	...
	int irenshu = StringToInt(message);
	if(irenshu > 0 && irenshu <= 16)
	{
		if(irenshu <= 4)
		{
			SetConVarInt(g_cvMinPlayers, irenshu);
			SetConVarInt(g_cvExtraPlayers, 0);
		}
		else 
		{
			SetConVarInt(g_cvMinPlayers, 4);
			SetConVarInt(g_cvExtraPlayers, irenshu - 4);
		}
		SetConVarInt(g_cvTeamLimit, irenshu);
		PrintToChat(client, "\x04[提示]\x05人数改为: %d.", irenshu);
		SetConVarString(g_cvRenshu, message);
	}
	...
	if (name[0] == 'a') {
		if (name[4] == 'a') {
			...
			if (name[5] == 'a' && strcmp(name, "abm_tankchunkhp") == 0) {
				...
				int count = CountTeamMates(2, 1) - g_TeamLimit;
				//PrintToChatAll("切走的数量%d", count);
				if(count > 0)
				{
					CreateTimer(0.5, ChangeOtherClient, count);
				}
				else
				{
					CreateTimer(0.5, ChangeOtherBot);
				}
			}
...
Action QTeamHookTimer(Handle timer, int client) {
	...
	PrintHintText(client, "输入 !jg 可以加入游戏");
	PrintToChat(client, "\x04[提示]\x05 输入 !jg 可以加入游戏");
	...
	if (GetQRecord(client)) {
		...
		switch (onteam) {
			case 2: {
				...
				//PrintToChatAll("加之前的人数: %d-%d", CountTeamMatesSc(2, 1), g_TeamLimit);
				if(CountTeamMatesSc(2, 1) < g_TeamLimit)
				{
					QueueUp(client, 2);
					AddSurvivor();
				}
				...
				{
					PrintHintText(client, "人数已满: %d 人 , 输入 !cm 可以调整最大人数", g_TeamLimit);
				}
			}
...
int CountTeamMatesSc(int onteam, int mtype=2, bool aliveonly=false) {
	Echo(2, "CountTeamMatesSc: %d %d", onteam, mtype);
	static int clients, bots, humans;
	clients = bots = humans = 0;
...
void RmBots(int asmany, int onteam) {
	...
	for (int i = MaxClients; i >= 1; i--) {
		...
		if (IsClientConnected(i) && IsFakeClient(i) && GetClientTeam(i) == onteam) {
...
public void TeleportClientHandler(int client, int level) {
	...
	switch (level) {
		...
		case 1: {
			...
			Format(g_sB, sizeof(g_sB), "%s: 传送玩家", g_sB);
...
Action SwitchTeamCmdSc(int client, int args) {
	Echo(2, "SwitchTeamCmd: %d", client);
...
public void SwitchTeamHandler(int client, int level) {
	...
	switch (level) {
		case 0: TeamMatesMenu(client, "选择玩家队伍", 1);
		case 1: {
			GetClientName(menuArg0, g_sB, sizeof(g_sB));
			Format(g_sB, sizeof(g_sB), "%s: 选择队伍", g_sB);
...
Action AssignModelCmdSc(int client, int args) {
	Echo(2, "AssignModelCmd: %d", client);
	menuArg0 = client;
	AssignModelHandler(client, 1);
	return Plugin_Handled;
}
...
public void AssignModelHandler(int client, int level) {
	...
	switch (level) {
		...
		case 1: {
			GetClientName(menuArg0, g_sB, sizeof(g_sB));
			Format(g_sB, sizeof(g_sB), "%s: 设置模型", g_sB);
...
public void SwitchToBotHandler(int client, int level) {
	...
	switch (level) {
		case 0: TeamMatesMenu(client, "接管电脑", 1);
		case 1: {
			GetClientName(menuArg0, g_sB, sizeof(g_sB));
			Format(g_sB, sizeof(g_sB), "%s: 接管", g_sB);
...
public void RespawnClientHandler(int client, int level) {
	...
	switch (level) {
		...
		case 1: {
			GetClientName(menuArg0, g_sB, sizeof(g_sB));
			Format(g_sB, sizeof(g_sB), "%s: 复活", g_sB);
...
public void CycleBotsHandler(int client, int level) {
	...
	switch (level) {
		...
		case 1: {
			GetClientName(menuArg0, g_sB, sizeof(g_sB));
			Format(g_sB, sizeof(g_sB), "%s: 切换机器人", g_sB);
...
public void StripClientHandler(int client, int level) {
	...
	switch (level) {
		case 0: TeamMatesMenu(client, "移除玩家装备", 2, 1);
		case 1: {
			GetClientName(menuArg0, g_sB, sizeof(g_sB));
			Format(g_sB, sizeof(g_sB), "%s: 移除玩家装备", g_sB);
...
void MainMenu(int client, char [] title) {
	...
	menu.AddItem("6", "移除玩家装备");  //"Upsticky");		// strip
...
public Action ChangeOtherClient(Handle timer, int count)
{
	for(int i = 1; i <= MaxClients + 1; i++)
	{
		if(IsValidClient(i) && IsClientConnected(i) && ((!IsFakeClient(i) || iHasIdlePlayer(i)) || !IsPlayerAlive(i)) && GetClientTeam(i) == 2)
		{
			if(IsFakeClient(i))
			{
				KickClient(i);
	...
	//CreateTimer(0.2, ChangeOtherBot);
	return Plugin_Handled;
}
public Action ChangeOtherBot(Handle timer)
{
	int count = CountTeamMates(2, 0);
	//PrintToChatAll("提走电脑: %d-%d",  count, g_TeamLimit);
	if(count > g_TeamLimit)
	{
		RmBots(count, 2);
	}
