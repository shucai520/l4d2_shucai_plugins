...
#define ROCK_AIM_TIME 2.3									// 坦克扔石头瞄准的时间
...
enum struct struct_TankConsume
{
	...
	void struct_Init()
	{
		this.bIsThrowingRock = this.bCanInitPos = this.bCanConsume = this.bIsReachingFunctionPos = this.bIsReachingRayPos = this.bInConsumePlace = this.bHasRecordProgress = this.bHasCreatePosTimer = false;
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	if (IsAiTank(client))
	{
		...
		if (IsValidSurvivor(target))
		{
			...
			if (g_hAllowThrow.BoolValue && throw_min_range >= 0 && throw_max_range >= 0)
			{
				...
				if (targetdist <= throw_max_range && targetdist >= throw_min_range)
				{
					...
					if(GetEntPropEnt(client, Prop_Send, "m_customAbility") != -1)
					{
						buttons |= IN_ATTACK2;
		...
		{
			if(eTankStructure[client].bIsThrowingRock && !Player_IsVisible_To(client, nearest_target))
			{
				nearest_target = GetClosetMobileCanSeeSurvivor(client);
				nearest_targetdist = GetClosetSurvivorDistance(client, nearest_target);
			}
			if (IsValidSurvivor(nearest_target))
			{
				...
				//落差
				float drop = self_eye_pos[2] - targetpos[2];
				
				//特感比人高100 drop > 100
				
				//if(drop > 50.0)
				if(nearest_targetdist > 80)
				{
					targetpos[2] += 45.0;
				}
				...
				if(nearest_targetdist > 350)
				{
					if(drop < 300.0)
					{
						//每130距离 +45投抛高度 
						targetpos[2] += 50.0 + (((nearest_targetdist - 750) / 130) * 50.0);
					}
				}

				//加点预判
				float fSpeed[3], fCurrentSpeed;
    			GetEntPropVector(nearest_target, Prop_Data, "m_vecVelocity", fSpeed);
				fCurrentSpeed = SquareRoot(Pow(fSpeed[0], 2.0) + Pow(fSpeed[1], 2.0));

				if(fCurrentSpeed > 50)
				{
					if(nearest_targetdist> 300 && nearest_targetdist < 520)
					{
						targetpos[0] += fSpeed[0] / 3;
						targetpos[1] += fSpeed[1] / 3;
					}
					else if(nearest_targetdist >= 520)
					{
						targetpos[0] += fSpeed[0] / 2;
						targetpos[1] += fSpeed[1] / 2;
					}

					//上升或落下速度 每250调整投抛高度10
					float tempF = FloatAbs(fSpeed[2]);
					int addnum = 0;
					if(tempF > 250.0)
					{
						int num = fSpeed[2] < 0 ? -1 : 1;
						targetpos[2] += (addnum + (addnum * (tempF / 250))) * num;
					}

					//targetpos[2] += fSpeed[2] / 2;
				}
				...
				//PrintToChatAll("目标: %N, 距离: %d,  落差: %f", nearest_target, nearest_targetdist, self_eye_pos[2] - targetpos[2]);
			}
		}
		//抛石头前诱惑视角
		else if(eTankStructure[client].bIsThrowingRock)
		{
			float self_eye_pos[3] = {0.0};
			GetClientEyePosition(client, self_eye_pos);
			self_eye_pos[0] *= -1;
			self_eye_pos[1] *= -1;
			self_eye_pos[2] *= -1;
			TeleportEntity(client, NULL_VECTOR, self_eye_pos, NULL_VECTOR);
		}
...
{
	eTankStructure[tank].bIsThrowingRock = false;
	return Plugin_Continue;
}
// 坦克跳砖
	...
	if (IsAiTank(client))
	{
		...
		if (flags & FL_ONGROUND && IsValidSurvivor(target))
		{
			if (!eTankStructure[client].bCanConsume)
			{
				...
				eTankStructure[client].bIsThrowingRock = true;
			}
			else
			{
